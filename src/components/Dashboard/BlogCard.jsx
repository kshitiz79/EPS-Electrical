...
const BlogCard = ({ blog, onEdit, onDelete }) => {
    ...
    return (
        <div className="group relative bg-[#06488d] text-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 border border-gray-100 flex flex-col h-full transform hover:-translate-y-1">
            {/* Image Container */}
            ...
            <div className="p-6 flex flex-col flex-grow">
                <div className="flex justify-between items-start mb-2">
                    <span className="text-xs font-semibold text-white bg-[#0a1f55]/5 px-2 py-1 rounded-md">
                ...
                <h3 className="text-xl font-bold  mb-3 line-clamp-2 leading-tight group-hover:text-[#0a1f55] transition-colors">
                    {title}
                </h3>
                <p className=" text-sm mb-6 line-clamp-3 leading-relaxed flex-grow">
                    {displayContent}
                </p>
                ...
                <div className="pt-4 border-t border-gray-100 mt-auto flex justify-between items-center">
                    <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center gap-2 text-white font-semibold text-sm hover:text-[#0a1f55]/80 transition-colors"
                    ...
                    {(onEdit || onDelete) && (
                        <div className="flex gap-2">
                            {onEdit && (
                                <button
                                    onClick={() => onEdit(blog)}
                                    className="p-2 text-white hover:text-blue-600 hover:bg-blue-50 rounded-full transition-colors"
                                    title="Edit Blog"
                                >
